...
This was done previously and does not need to be repeated if files are available
...
```{r}
#vol_df.edit <- vol_df.edit <- vol_df.edit %>% mutate_at(vars(ends_with(".padj")), ~ ifelse(adp < 1e-30, 1e-30, .))
```
...
```{r}
ssGSEA.project.dataset(input.ds = "d7_normCounts.gct", output.ds = "h_ssGSEA", gene.sets.dbfile.list = "gene_sets/h.all.v2023.2.Hs.symbols.gmt")
ssGSEA.project.dataset(input.ds = "d7_normCounts.gct", output.ds = "c2cp_ssGSEA", gene.sets.dbfile.list = "gene_sets/c2.cp.v2023.2.Hs.symbols.gmt")
ssGSEA.project.dataset(input.ds = "d7_normCounts.gct", output.ds = "c2cgp_ssGSEA", gene.sets.dbfile.list = "gene_sets/c2.cgp.v2023.2.Hs.symbols.gmt")
ssGSEA.project.dataset(input.ds = "d7_normCounts.gct", output.ds = "c5bp_ssGSEA", gene.sets.dbfile.list = "gene_sets/c5.go.bp.v2023.2.Hs.symbols.gmt")
ssGSEA.project.dataset(input.ds = "d7_normCounts.gct", output.ds = "c5mf_ssGSEA", gene.sets.dbfile.list = "gene_sets/c5.go.mf.v2023.2.Hs.symbols.gmt")
ssGSEA.project.dataset(input.ds = "d7_normCounts.gct", output.ds = "c5cc_ssGSEA", gene.sets.dbfile.list = "gene_sets/c5.go.cc.v2023.2.Hs.symbols.gmt")
```
...
```{r}
rnkFile <- assembleDat %>% filter(MPG == "Yes" & gene_type == "protein_coding") %>% select('gene_name',ends_with(".stat"))
...
for (i in names[-1]){
  ...
  write.table(d, sep='\t',file=paste0("../gsea/",i,".rnkTT"),col.names=TRUE, quote=FALSE, row.names=FALSE)
}
```

## Process HPC tsv GSEA results to produce a single output file

Run bash script submit_prerank_4.3.2_v2023.2.sh to loop through each rank file for each gene set collection
